                    ...
                    @if($edit_attr->parent !=0)
                        <div class="mb-3">
                            ...
                            <div id="attr_color" style=" @if($edit_attr->color) display:block @endif">
                                ...
                                <input type="text" value="{{ $edit_attr->color }}" value="{{ $edit_attr->color }}" />
                            ...
                            <div id="attr_image"  style=" @if($edit_attr->image) display:block @endif">
                                <input type="hidden" class="admin-media-frame-input " name="thumbnail" value="{{ $edit_attr->image }}">
                ...
                <table class="admin-table final-table">
                    ...
                    @foreach($attributes as $attribute)
                        ...
                        <tr>
                            ...
                            <td>
                                @if($attribute->image)
                                    <img src="{{ asset(store_image_link($attribute->image)) }}" width="30px" height="30" style="margin-left: 8px;">
                                @elseif($attribute->color)
                                    <div style="background-color: {{ $attribute->color }};width: 30px;height: 30px;margin-left: 8px;display: inline-block;border-radius:5px"></div>
                                @endif
                                <a href="{{ route("admin_edit_attribute_value",["id"=>$attribute->id]) }}">{{ $attribute->name }}</a>

                                <div class="admin-table-actions">

                                    <a href="{{ route("admin_edit_attribute_value",["id"=>$attribute->id]) }}"><span>بروزرسانی</span></a>
